...
| `/audio`      | Playback controls (play/pause, upload fallback). **Audio controls are ready for live wiring** to `/api/audio/devices` (list), `/api/audio/{id}/play|stop|volume|upload`. UI should bind to fields: `{status, volumePercent, playback.state, capabilities}` from device payloads. | `AudioModule` + `mockApi.audio()` (transitioning to live).                                                   |
| `/video`      | HDMI/CEC control layout with power, input, volume cards.                                                 | `VideoModule` + `mockApi.video()`.                                                                           |
| `/zigbee`     | **External**: Full-page Zigbee2MQTT UI proxied by Caddy. Not a native Fleet module.                     | Proxied to `pi-video-01:8084` via Caddy (tailnet-restricted).                                               |
| `/camera`     | Camera summary card with placeholder preview/log link.                                                   | `CameraModule` + `mockApi.camera()`.                                                                         |
| `/files`      | Asset file manager proxied through Caddy (File Browser).                                                | Proxied to filebrowser service via `/files/*` route.                                                        |
