...
test.describe('Audio Device Controls', () => {
  ...
  test('shows fallback status for each device', async ({ page }) => {
    ...
    await expect(card1.getByText('Fallback: Ready')).toBeVisible();
    await expect(card2.getByText('Fallback: None')).toBeVisible();
  });
  ...
  test('shows console errors are suppressed', async ({ page }) => {
