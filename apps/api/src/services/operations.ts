...
export async function executeOperation(
  ...
  try {
    ...
    metrics.circuit_breaker_state.set({ deviceId }, 0);
    ...
    if (isAxiosError(error)) {
      ...
      const reason = error.code || String(error.response?.status || 'axios');
      metrics.upstream_device_failures_total.labels(deviceId, reason).inc();
      metrics.circuit_breaker_state.set({ deviceId }, 1);
